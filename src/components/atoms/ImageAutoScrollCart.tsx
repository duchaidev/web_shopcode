...
const ImageAutoScrollCart: FC<{
  ...
  const handleMouseOver = () => {
    if (imageRef.current) {
      const imageHeight = imageRef.current.scrollHeight - 165 // Cuộn đến cuối ảnh trừ đi chiều cao của container
      const step = () => {
        if (imageRef.current) {
  ...
  return (
    <div
      className={`w-[100%] h-[165px] overflow-hidden relative`}
...
export default ImageAutoScrollCart
